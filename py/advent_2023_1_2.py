...
reverse_numbers = {k[::-1]:v for k, v in numbers.items()}

digits = {str(x): x for x in range(1, 10)}

def get_one_number(line, number_dict):
    first_index, ans = len(line), 0
    for k, v in (digits | number_dict).items():
        idx = line.find(k)
...
def get_number(line):
    left = get_one_number(line, numbers)
    right = get_one_number(line[::-1], reverse_numbers)
